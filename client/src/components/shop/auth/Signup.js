    ...
    if (data.cPassword !== data.password) {
      return setData({
        ...
        error: {
          ...
          email: "Please enter you SJSU email ID",
          sjsuid: "Please enter valid SJSU ID"
        },
      });
    }
    if (/^\w+([\.-]?\w+)*@sjsu.edu/.test(data.email)){
        console.log("yup sjsu id");
    }
    else{
        console.log("nope not sjsu");
    }
    try {
  ...
  return (
    <Fragment>
      ...
      <form className="space-y-4">
        ...
        <div className="flex flex-col">
          <label htmlFor="sjsuid">
            SJSU ID <span className="text-sm text-gray-600 ml-1">*</span>
          </label>
          <input
            onChange={(e) =>
              setData({
            ...
            id="sjsuid"
            className={`${
              data.error.sjsuid ? "border-red-500" : ""
            } px-4 py-2 focus:outline-none border`}
          />
          {!data.error ? "" : alert(data.error.email, "red")}
        </div>
